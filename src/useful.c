...
int isTarRelated(char *path){
  unsigned int size = strlen(getcwd(NULL,0)) + strlen(getenv("TARNAME"))
  + strlen(getenv("TARPATH")) + strlen(path) + 3;
  char s[size];
  memset(s,0,size);
  strcat(s,getcwd(NULL,0));
  if(strlen(getenv("TARNAME")) > 0) strcat(s,"/");
  strcat(s,getenv("TARNAME"));
  if(strlen(getenv("TARPATH")) > 0) strcat(s,"/");
  ...
  return strstr(simplified,".tar") != NULL;
}
