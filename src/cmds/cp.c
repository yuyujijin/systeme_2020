...
int cp_r(char **argv){
  ...
  for(int i = 0; i < index; i++){
    // On concatene tout
    char argv1[strlen("..") + 1 + strlen(argv[1]) + 1 + strlen(filenames[i]) + 1];
    memset(argv1,0,strlen("../") + strlen(argv[1]) + 1 + strlen(filenames[i]) + 1);
    sprintf(argv1,"../%s/%s",argv[1],filenames[i]);

    char *newargv[2] = {filenames[i],argv1};
    int w;
